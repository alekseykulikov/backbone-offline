...
(function() {
  ...
  describe('Offline.Storage', function() {
    ...
    describe('isLocalStorageSupport', function() {
      it('should returns false when raised error', function() {
        spyOn(localStorage, 'setItem').andThrow('QUOTA_EXCEEDED_ERR');
        return expect(this.storage.isLocalStorageSupport()).toBeFalsy();
      });
      return it('should returns true for modern browsers which support localStorage', function() {
        return expect(this.storage.isLocalStorageSupport()).toBeTruthy();
      });
    });
    describe('setItem', function() {
      it('should sets item to localStorage', function() {
        this.storage.setItem('key', 'value');
        return expect(this.storage.getItem('key')).toEqual('value');
      });
      return describe('when localStorage has errors', function() {
        it("should calls 'quota_exceed' when item exceeding browser quota", function() {
          var quotaCallback;
          quotaCallback = jasmine.createSpy('-Quota Exceeded Error-');
          spyOn(localStorage, 'setItem').andThrow({
            name: 'QUOTA_EXCEEDED_ERR'
          });
          this.storage.collection.on('quota_exceed', quotaCallback);
          this.storage.setItem('key', 'value');
          return expect(quotaCallback.callCount).toBe(1);
        });
        return it('should sets @support to false in other cases', function() {
          spyOn(localStorage, 'setItem').andThrow('UNKNOWN_ERR');
          this.storage.setItem('key', 'value');
          return expect(this.storage.support).toBeFalsy();
        });
      });
    });
    ...
    describe('mid', function() {
      return it("should return 24 hex digits", function() {
        return expect(this.storage.mid()).toMatch(/[a-f,0-9]{24}/);
      });
    });
    describe('clear', function() {
      ...
      it('storage should be empty', function() {
        this.storage.clear();
        return expect(this.storage.isEmpty()).toBeTruthy();
      });
