...
(function() {
  ...
  describe('Offline.Sync', function() {
    ...
    describe('ajax', function() {
      describe('when onLine', function() {
        beforeEach(function() {
          ...
          spyOn(this.sync, 'prepareOptions');
          return this.sync.ajax('read', this.dreams, {});
        });
        it('should calls Backbone.ajaxSync', function() {
          return expect(Backbone.ajaxSync).toHaveBeenCalledWith('read', this.dreams, {});
        });
        return it('should runs prepareOptions', function() {
          return expect(this.sync.prepareOptions).toHaveBeenCalled();
        });
      });
      return describe('when offline', function() {
        return it('should sets offline', function() {
          Offline.onLine.andReturn(false);
          this.sync.ajax('read', this.dreams, {});
          return expect(localStorage.getItem('offline')).toEqual('true');
        });
      });
    });
    describe('full', function() {
      ...
      it('should not reset collection if silent is passed', function() {
        var resetCallback;
        resetCallback = jasmine.createSpy('-Reset Callback-');
        this.sync.collection.items.on('reset', resetCallback);
        this.options.silent = true;
        this.sync.full(this.options);
        return expect(resetCallback.callCount).toBe(0);
      });
      ...
      it('should request data from server', function() {
        spyOn($, 'ajax');
        this.sync.full(this.options);
        return expect($.ajax.mostRecentCall.args[0].url).toEqual('/api/dreams');
      });
    ...
    describe('prepareOptions', function() {
      ...
      it('should clears oflline item', function() {
        this.sync.prepareOptions({});
        return expect(this.storage.getItem('offline')).toEqual(null);
      });
      return it('should change success function added call of incremental sync', function() {
        var options, successCallback;
        successCallback = jasmine.createSpy('-Success Callback-');
        spyOn(this.sync, 'incremental');
        options = {
          success: successCallback
        };
        this.sync.prepareOptions(options);
        options.success();
        expect(successCallback.callCount).toBe(1);
        return expect(this.sync.incremental).toHaveBeenCalled();
      });
    });
